...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        self.vars.create_var('waypoints_x_previous', 0)
        self.vars.create_var('waypoints_y_previous', 0)
        ...
        if self._start_control_loop:
            ...
            P = 1.2 * speed_error
            integral_error = self.vars.integral_error_previous + speed_error * delta_time
            I = 0.2 * integral_error
            ...
            if self._current_frame < len(waypoints) - 1:
                waypoint_current_x = waypoints[self._current_frame][0]
                waypoint_current_y = waypoints[self._current_frame][1]
                waypoint_next_x    = waypoints[self._current_frame + 1][0]
                waypoint_next_y    = waypoints[self._current_frame + 1][1]
            else:
                waypoint_current_x = waypoints[-2][0]
                waypoint_current_y = waypoints[-2][1]
                waypoint_next_x    = waypoints[-1][0]
                waypoint_next_y    = waypoints[-1][1]

            current_waypoints = np.array([[waypoint_current_x, waypoint_current_y]])

            # heading error
            ref_heading_angle = np.arctan2(waypoint_current_y - self.vars.waypoints_y_previous, waypoint_current_x - self.vars.waypoints_x_previous)
            heading_error = (ref_heading_angle - yaw)
            
            # crosstrack error
            path_vector = np.array([waypoint_next_x - waypoint_current_x, waypoint_next_y - waypoint_current_y])
            car_vector  = np.array([x - waypoint_current_x, y - waypoint_current_y])

            crosstrack_distance = (
                     -path_vector[0] * car_vector[1]
                    + path_vector[1] * car_vector[0]
                ) / np.linalg.norm(path_vector)*2
            
            crosstrack_error = np.arctan(0.3 * crosstrack_distance / v)

            steer_output = heading_error + crosstrack_error
            ...
            #print("crosstrack_error: ", crosstrack_error * 180/np.pi)
        ...
        self.vars.waypoints_x_previous = waypoint_current_x
        self.vars.waypoints_y_previous = waypoint_current_y
